#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000, ONLY AUXILIARY LOSS, c_index= ？ remove log_var[i] seems as no difference.
"""
...
#    out_gene=Bidirectional(LSTM(55,return_sequences=False), merge_mode='concat')(gene_input)
    ...
    out_gene=Bidirectional(LSTM(100,activation='tanh', return_sequences=False,\
...
#    model =Model([gene_input,clinic_input,y1_true, y2_true], out)
    ...
    model.compile(optimizer='adam', loss=ordinal_loss0 )
...
#    plt.savefig('Loss Curves only main loss.jpg', dpi=300) #指定分辨率保存
#    plt.show()
    
    
    np.savetxt("GBMLGG_hist.history['loss1000_ml_ordCOX20201220']_without_penalty_term_only_auxiliary_loss.csv",hist.history['loss'], delimiter=",")
...
np.savetxt("GBMLGG_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv", ypred, delimiter=",")
np.savetxt("GBMLGG_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv", time_new, delimiter=",")
np.savetxt("GBMLGG_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv", status_new, delimiter=",")
np.savetxt("GBMLGG_ypred_train_median1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv", ypred_train, delimiter=",")

df = pd.read_csv("GBMLGG_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv",header=None)    
month=np.asarray(pd.read_csv("GBMLGG_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv",header=None)) 
status=np.asarray(pd.read_csv("GBMLGG_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_AUXlloss.csv",header=None)) 
...
np.save("c_indices_without penalty term_only_AUXlloss",c_indices_mlp) 
print("GBMLGG_methylation_mRNA_lmqcm_lr_backend2233_only_auxiliary_loss20210123_.py") 
print("GBMLGG only_AUXloss:dataX=data_rna+methylation:",c_indices_mlp)
data_a=np.load('c_indices_without penalty term_only_AUXlloss.npy')
aa=0